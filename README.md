...
Providers (`LLM_PROVIDER`): `gemini` — the intended production model, reached through Google's OpenAI-compatible endpoint with `GEMINI_API_KEY` (same hardened provider code as `openai`, JSON mode, temperature 0, low reasoning effort; `GEMINI_BASE_URL` only if you proxy it); `openai` (any OpenAI-compatible Chat Completions endpoint via `OPENAI_BASE_URL`); `anthropic` (Messages API); and `template` — a deterministic, evidence-only renderer with no network call, used for zero-credential dry runs and as an honest fallback. Model requests use a separate 180-second timeout and retry only transient outages up to four total attempts; an exhausted Gemini outage falls back to the grounded template while permanent/configuration errors still fail loudly. `LLM_MODEL` is required and never hardcoded (pick a current, inexpensive Gemini model id from Google's model list).
...
| `ARKHAM_LLM_TIMEOUT_SECONDS` | Per-attempt analyst-model timeout; does not affect source fetching | `180` (range: 1–300) |
...
* A run makes zero model calls with `LLM_PROVIDER=template`, or normally one request with Gemini/OpenAI/Anthropic. Transient provider failures can raise that to four attempts before failing or using Gemini's deterministic fallback. Token volume varies with the collected evidence.
